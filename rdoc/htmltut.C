...
// It creates gDocdir if not already there
// It creates gDocdir/tutorials if not already there
// It creates gDocdir/tutorials/index.html (index to all directory tutorials)
// It creates gDocdir/tutorials/dir/index.html with index of tutorials in dir
// It creates gDocdir/tutorials/dir/*.C.html with one html file for each tutorial
// Author: Rene Brun
...
//           title for the previous and next links.
   ...
   if (name) {
      ...
      links += TString::Format("<div class=\"location\"><h2>From <a href=\"http://root.cern.ch/gitweb?p=root.git;a=blob;f=tutorials/%s;hb=HEAD\">$ROOTSYS/tutorials/%s</a></h2></div>",
               suburl.Data(), subtitle.Data());
   } else {
      if (suburl != "")
         suburl = TString("/") + suburl;
      links += TString::Format("<div class=\"location\"><h2>From <a href=\"http://root.cern.ch/gitweb?p=root.git;a=tree;f=tutorials%s;hb=HEAD\">$ROOTSYS/tutorials/%s</a></h2></div>",
               suburl.Data(), subtitle.Data());
   }
}
...
void writeTutorials(THtml& html) {
   ...
   TString indx = gDocdir + "/tutorials/index.html";
   ofstream fptop(indx);
...
Bool_t IsNew(const char *filename) {
   // Check if file in Git is changed within last 6 months
   static const char *months[] = {"Jan","Feb","Mar","Apr","May",
                                  "Jun","Jul","Aug","Sep","Oct",
                                  "Nov","Dec"};
   TString fn(filename);
   fn.ReplaceAll(TString(gRootDir)+"/", "");
   gSystem->Exec(Form("git --git-dir=%s/.git log -n 1 -- %s > MakeTutorials-tmp.log",
                 gRootDir, fn.Data()));
   ...
   Int_t year,month=0,day,x1,x2,x3;
   char dow[10], sm[10];
   while (fgets(line,240,fpdate)) {
      const char *com = strstr(line,"Date: ");
      if (com) {
         sscanf(&com[8],"%s %s %d %d:%d:%d %d",dow,sm,&day,&x1,&x2,&x3,&year);
         for (int i = 0; i < 12; i++) {
...
void scandir(THtml& html, const char *dir, const char *title, TObjLink* toplnk) {
   ...
   TString fullpath = gDocdir + "/tutorials/";
   fullpath += dir;
   ...
   TString outpath = gDocdir + "/tutorials/";
   outpath += dir;
...
void htmltut(const char *docdir)
{
   ...
   gDocdir = docdir;
   TString tutdir = gDocdir + "/tutorials";
   if (!gSystem->OpenDirectory(gDocdir)) gSystem->MakeDirectory(gDocdir);
   if (!gSystem->OpenDirectory(tutdir)) gSystem->MakeDirectory(tutdir);
   gEnv->SetValue("Unix.*.Root.Html.SourceDir", "$(ROOTSYS)");
   gEnv->SetValue("Root.Html.ViewCVS","http://root.cern.ch/gitweb?p=root.git;a=blob;f=%f;hb=HEAD");
   gEnv->SetValue("Root.Html.Search", "http://www.google.com/search?q=%s+site%3A%u");
   THtml html;
   html.SetOutputDir(gDocdir);
   html.LoadAllLibs();
